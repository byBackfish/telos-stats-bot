...
export default class PlayerTrackCommand extends BunCommand<CustomClient> {
  ...
  public async execute(
    ...
    { player, type, time }: { player: string; type: string | undefined, time: string },
  ): Promise<CommandReturnable> {
    if(!type) {
        const firstType = this.client.leaderboardTypes.find(stat => stat.category === 'player');
        if(!firstType) return;
        type = firstType.name;
    }
  ...
  async sendDataMessage(interaction: CommandInteraction, id: string, type: string, player: string, time: string) {
    ...
    let isUuid = player.length > 16;
    let query = isUuid ? {uuid: player} : {username: player};

    const oldData = await getStatAtTime<number>(query, stat, thenTime);
    if(!oldData) return interaction.editReply({content: `Player's old data could not found`});

    const newData = await getStatNow<number>(query, stat);
    if (!newData) return interaction.editReply({content: `Player's data could not found`});
    ...
    const embed = new EmbedBuilder()
        ...
        .setTimestamp()
        .setDescription(`Found Saved Data from: <t:${Math.round(oldData.time.getTime() / 1000)}:R>`)
        .setFooter({
    ...
    this.client.await<StringSelectMenuInteraction>(`player-track-category-${id}`, 1).then((select) => {
        if(select.values.length === 0) {
            return select.reply({content: 'You must select a category', ephemeral: true});
        }
        select.deferUpdate();
        this.sendDataMessage(interaction, response.id, select.values[0], player, time);
    })

    this.client.await<StringSelectMenuInteraction>(`player-track-time-${id}`, 1).then((select) => {
        if(select.values.length === 0) {
            return select.reply({content: 'You must select a time', ephemeral: true});
        }
        select.deferUpdate();
        this.sendDataMessage(interaction, response.id, type, player, select.values[0]);
    })
  }

  buildCategorySelection(selectedCategory: string, selectedTime: string, id: string): ActionRowBuilder[] {
    ...
    for (let i = 0; i < 25; i += 25) {
      ...
      const builder = new StringSelectMenuBuilder().addOptions(options).setMinValues(1).setMaxValues(1).setCustomId(`player-track-category-${id}`).setPlaceholder('Select a category');
    ...
    for (let i = 0; i < TIME_OPTIONS.length; i += 25) {
      ...
      const builder = new StringSelectMenuBuilder().addOptions(options).setMinValues(1).setMaxValues(1).setCustomId(`player-track-time-${id}`).setPlaceholder('Select a time');
