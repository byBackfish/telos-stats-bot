...
export class DataSavingService {
    constructor(private playerServcie: PlayerService, private options: DataSavingOptions) {}
    ...
    private async save() {
        console.log(`Saving data for ${this.playerServcie.getPlayers().length} uuids`);
        console.log(`Progress: 0/${this.playerServcie.getPlayers().length} | Last: None`);
        let i = 0;
        const options: StreamOptions<string, PlayerProfile> = {
            ...
            callback: async (data) => {
                ...
                i++;
                console.log(`Progress: ${i}/${this.playerServcie.getPlayers().length} | Last: ${data.username}`);
            },
