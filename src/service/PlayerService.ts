...
export class PlayerService {
	...
	async loadPlayers() {
		...
		if (
			(lastSavedUuids &&
				Date.now() - lastSavedUuids.time < this.options.cacheTimeout) ||
			!this.options.shouldRefreshCache
		) {
		...
		const untilNextRefresh =
			this.options.cacheTimeout - (Date.now() - lastSavedUuids.time);
