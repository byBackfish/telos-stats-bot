...
const createRaidProvider = (
	...
	return createProvider(`Raid Leaderboard - ${raidName}`, async () => {
		...
		return Object.values(json).map(
			(entry: any) =>
				({ uuid: entry.uuid, username: entry.name }) satisfies ProvidedPlayer,
		);
	});
};
...
const createGuildProvider = (guildName: string): PlayerNameProvider => {
	return createProvider(`Guild - ${guildName}`, async () => {
		const dataByUUID = await getGuildProfile({
			name: guildName,
			playerReturnIdentifier: "uuid",
		});

		const getAllMembers = (
			data: GuildProfile<"uuid">,
		): (Member<"uuid"> & { uuid: string })[] => {
			const members = [
				data.members.recruit,
				data.members.recruiter,
				data.members.captain,
				data.members.strategist,
				data.members.chief,
				data.members.owner,
			].flatMap((rows) => {
